...
func (http *httpAdder) serveAdd(ctx *fiber.Ctx) (err error) {
	return http.serveMethod(ctx, "add", http.add)
}
func (http *httpAdder) add(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {
	var err error
	var request requestAdderAdd
	...
	response.C, err = http.svc.Add(methodContext, request.A, request.B)
	if err != nil {
